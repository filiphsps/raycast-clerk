...
- **Clerk Menu Bar** — optional menu-bar item showing the active instance, with quick app switching and command launchers. Disable it from its menu ("Disable Menu Bar"); run the command again from Raycast's root search to re-enable. From its menu you can also switch the title between name and icon-only (useful for long app names).
